...
function ProjectImage({ src, alt, immediate = false, delay = 0 }: { src: string; alt: string; immediate?: boolean; delay?: number }) {
  const ref = useRef(null)
  const scrollInView = useInView(ref, { once: true, margin: '-60px' })
  ...
  return (
    <div ref={ref} className="relative aspect-[4/3] w-full overflow-hidden rounded-xl">
      <motion.div
        ...
        transition={{ duration: 0.9, delay: inView && immediate ? delay : 0, ease: [0.16, 1, 0.3, 1] }}
      >
        <motion.div
    ...
    <section id="work" className="py-24">
      ...
      <div className="flex flex-col gap-0">
        {selectedWork.map((project, i) => (
          <AnimateIn key={project.id} delay={i === 0 ? 1.1 : 0.08} immediate={i === 0}>
            <div id={project.id} className="border-t border-[var(--border)]">
              <ProjectImage src={project.image} alt={project.title} immediate={i === 0} delay={0.2} />
